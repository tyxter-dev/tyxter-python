...
Checked: 2026-09-03
Baseline SHA: `332b21afa0dc890ef2e8f0cb70ef5ca7e9d1bdb6`
...
|---|---|---|---|
| Worktree and branch | Dedicated clean branch; automation branch untouched; approved plan present before dispatch | Managed worktree `codex-sdk-js-0.8-parity-a0889dda` exists at the baseline SHA and is clean; draft plan is still in the authoring checkout pending approval/commit activation below | ready |
...
| Full local gate | Each accepted section or correction | `1 / 1` per acceptance round | proven — baseline gate passed | 0 | It is local; each public slice can invalidate typing, conformance, or packaging |
...
flowchart LR
  ...
  subgraph PA["Phase A — source and contract parity"]
    ...
    A2["A2 — message and media truthfulness ⚠"]
    A3["A3 — transcription recovery observability ⚠"]
    A4["A4 — template authoring parity ⚠"]
    A5["A5 — phone review state parity ⚠"]
    A6["A6 — provider availability parity ⚠"]
...
- None at plan-review time; append factual implementation corrections here at each acceptance.
...
- [ ] A1 Adopt PR #2 and close all eight route gaps — all Goal 2 clauses green — profile:
      `goal-implementer-terra` — routing: requested=Terra/xhigh; role=pending production dispatch;
      model/effort=runtime metadata unavailable; fallback=none
- [ ] A2 Mirror JavaScript 0.8 message and media truthfulness — Goal 1 message/media
      clauses green — profile: `goal-implementer-terra` — routing: requested=Terra/xhigh;
      role=pending; model/effort=pending runtime evidence; fallback=none
- [ ] A3 Mirror transcription recovery and failure observability — Goal 1 transcription/error
      clauses green — profile: `goal-implementer-terra` — routing: requested=Terra/xhigh;
      role=pending; model/effort=pending runtime evidence; fallback=none
- [ ] A4 Mirror template authoring parity — Goal 1 template clauses green — profile:
      `goal-implementer-terra` — routing: requested=Terra/xhigh;
      role=pending; model/effort=pending runtime evidence; fallback=none
- [ ] A5 Mirror phone review state parity — Goal 1 phone review/tier clause and reader sweep green — profile:
      `goal-implementer-terra` — routing: requested=Terra/xhigh;
      role=pending; model/effort=pending runtime evidence; fallback=none
- [ ] A6 Mirror provider availability and flow linkage parity — Goal 1 provider/flow clauses green — profile:
      `goal-implementer-terra` — routing: requested=Terra/xhigh;
      role=pending; model/effort=pending runtime evidence; fallback=none
- [ ] A7 Mirror promotional credit parity — Goal 1 promotional-credit clause green — profile:
      `goal-implementer-terra` — routing: requested=Terra/xhigh;
      role=pending; model/effort=pending runtime evidence; fallback=none
