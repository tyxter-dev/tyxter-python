...
gdi_version: 0.2.1
status: shipped
approval: "2026-09-03: user approved F1-F4, then explicitly authorized merging PR #3 and publishing tyxter 0.8.0"
harness: codex
---
...
- **Published 0.8 parity and current manifest parity have different provenance.** The 0.8 tag has
  179 manifest rows; PR #2 pins 181 rows at `ed49c514...`. The two extra phone-renewal reads ship in
  Python 0.8.0 as identified forward-compatible additions and are never attributed to the
  published JavaScript tag.
...
- **Repository and registry versions differed during planning.** Python source was 0.6.0 while PyPI
  served 0.4.0. After the verified candidate was merged under the user's follow-up authorization,
  Trusted Publishing released Python 0.8.0 on 2026-09-03.
...
| Credentials / external authority | Read canonical source and PR metadata; no release credential needed for implementation | Local authorized messaging checkout, npm/PyPI reads, and GitHub PR reads succeeded; publication was initially out of scope and later authorized by F5 | ready |
...
|---|---|---:|---|---:|---|
| Assembled public-client walkthrough | Public client/resources, builders, response types, signature verification | `0 / 1` | proven — baseline example test passed | 1 | Cheapest real-client-equivalent probe uses `Tyxter` plus `MockTransport`; run after final review and before the persistent build |
| Full local gate | Each accepted section or correction | `1 / 1` per acceptance round | proven — baseline gate passed | 26 | It is local; each public slice can invalidate typing, conformance, or packaging |
| Persistent wheel/sdist and fresh-install audit | Final source, version, metadata, exports, package allowlist | `0 / 1` | proven — baseline wheel imported in a Python 3.13 venv | 1 | Run once after whole-branch review and the assembled-client probe |
| Dedicated branch push + successor draft PR | Accepted commits and clean package audit | `0 / 1` | unproven — authority read only | 1 | Push only the fully reviewed candidate; never mutate the volatile sync branch |
| Exact-head GitHub CI matrix | Pushed successor head | `0 / 1` plus defect corrections | proven route — PR #2 exercised Python 3.10–3.13 | 1 | Run once after all local producers converge; corrections invalidate the count and are recorded |
| Supersede PR #2 | Successor URL, exact artifact inclusion, green successor CI | `0 / 1` | unproven | 1 | Link and close PR #2 only after its replacement is demonstrably safer |
| Merge successor PR #3 | Green exact-head CI and explicit follow-up authority | `0 / 0` initially | not authorized during planning | 1 | User explicitly expanded the terminal scope after verification; squash merge `cd77d2f` landed on `main` |
| Stable Trusted Publishing workflow | Merged main commit, version/channel agreement, duplicate guard, full matrix | `0 / 0` initially | not authorized during planning | 1 | User explicitly requested publication; run `33799345433` published 0.8.0 with OIDC attestations |
...
Post-review evidence: the assembled walkthrough passed once; `uv build` produced
`tyxter-0.8.0.tar.gz` and `tyxter-0.8.0-py3-none-any.whl` once; the wheel installed in a fresh
CPython 3.13.5 environment and imported the client, error base, webhook verifier, and version
`0.8.0`. The generated archives and environment were moved out of the worktree into HubGrid's
disposable artifact area after the audit.

Lifecycle counts track producer-invalidating gate executions. Automatic CI confirmations caused
only by terminal plan-ledger commits do not consume another producer gate, but the successor PR's
current head must still be green at handoff.
...
| F5 | post-verification | Merge and publication | retain verified draft / merge only / merge and publish stable 0.8.0 | merge the green successor, then use the repository Trusted Publishing workflow from `main`; do not upload manually or invent a patch version | ruled 2026-09-03: user explicitly requested merge and publication; F5 supersedes F4's terminal limit |
...
| R7 | post-release | Preserve the post-tag renewal provenance after publication | Once F5 shipped the artifact, the old `Unreleased` label became false; docs now identify the methods as shipped forward-compatible Python additions without attributing them to `sdk-js-v0.8.0` |
...
- Keep published JavaScript 0.8 behavior and post-tag forward-compatible behavior distinguishable
  in code review, changelog, and provenance evidence.
...
- [x] Every public method, request/response field, error detail, webhook envelope, exported type,
...
- [x] WhatsApp voice notes, truthful unsupported/unknown inbound messages, phone-less sender reads,
...
- [x] A strict negative typing fixture rejects Instagram `voice`, and a paired positive fixture plus
      runtime assertion accepts and serializes legitimate Instagram media without `voice`.
- [x] Blank transcription retry keys fail before network I/O, while a legitimate nonblank key is
      admitted and remains stable across the logical request.
- [x] Existing 0.6 Python callers remain runtime- and type-compatible; strict typing is not weakened
...
- [x] All 181 manifest rows are classified, with 179 typed SDK routes plus only the two existing
      reviewed media blob exemptions; there are no phantom, duplicate, deferred, or new exempted
      routes.
- [x] Projects, public feedback reads, webhook endpoint testing, and phone-renewal reads use exact
      paths, encoded IDs, query names, body omission, idempotency modes, and trace-header modes.
- [x] Both conformance files are byte-identical to messaging commit `ed49c514...`, and
      `SOURCE_COMMIT` contains that full SHA while the post-0.8 phone-renewal surface remains
      explicitly distinguished from the published JavaScript tag.

### Goal 3 — The Python 0.8 release is distributable and reviewable

- [x] Source version, changelog, README, route inventory, User-Agent behavior, and distribution
      assertions agree on 0.8.0 and now accurately identify the PyPI publication.
- [x] Ruff, strict mypy, all tests, whole-branch focused reviews, one wheel/sdist audit, and a fresh
      wheel import pass; the package includes the intended modules, public exports, `py.typed`,
      README, changelog, and license.
- [x] One dedicated successor PR passed exact-head CI on Python 3.10–3.13, safely superseded PR #2,
      merged to `main`, and published through the authorized stable Trusted Publishing workflow.
- [x] A scripted assembled-client walkthrough sends, inspects, and verifies a public webhook via
      `Tyxter` plus `MockTransport`, covering the final resource registry and signature helper.

The plan first reached `verified`; the user's later F5 authorization allowed it to reach `shipped`
through the repository's Trusted Publishing workflow.

Out of scope throughout: a live Tyxter API probe; async APIs or new dependencies; server/database
changes; and canonical changes after `ed49c514...`. Merge and publication were outside the original
scope but were later authorized by F5; no tag or manual PyPI upload was added.
...
flowchart LR
  ...
  G1 --> FR{"whole-branch final review — current origin/main ✅"}
  G2 --> FR
  LOCAL --> FR
  FR -- findings --> FIX["one Terra correction agent 🔁×1"]
  FIX --> LOCAL
  FR -- clean --> PROBE{"assembled public-client walkthrough ×1 ✅"}
  PROBE --> BUILD{"wheel/sdist + fresh install ×1 ✅"}
  BUILD --> G3{"Goal 3 — distributable release ✅"}
  ...
  SUPERSEDE --> MERGE["user-authorized squash merge ✅"]
  MERGE --> PUB["stable Trusted Publishing run ✅"]
  PUB --> HANDOFF(["shipped handoff — PyPI 0.8.0"])
```
...
- **Mixed provenance** — the published 0.8 tag has 179 manifest rows, while PR #2 pins 181 rows at
  ...
  SHAs and identifies those two methods as forward-compatible Python additions while using PR #2's
  fresher conformance truth; they are not attributed to the JavaScript tag.
...
- **Post-verification scope expansion** — F5 added one squash-merge gate and one stable publishing
  gate after the original plan reached `verified`. Their initial planned count was zero; both
  one-time executions are explicitly recorded and were authorized by the user before mutation.
...
- **Python had never published the 0.5–0.8 candidates during planning** — PyPI served 0.4.0 while
  repository source was 0.6.0. The verified cumulative 0.8.0 candidate was later published only
  after the user supplied the separate F5 authority.
...
- Confirmed: the volatile sync-branch handoff, mixed tag/current provenance boundary, and
  cross-language compatibility risk all fired. The dedicated successor preserved PR #2's exact
  artifacts; release notes kept post-tag renewals separate; whole-branch R1 restored callable
  Python construction for five additive response keys before outward gates.
- Never fired: no snapshot conflict, phantom/deferred route, implementer/reviewer routing failure,
  runtime infrastructure blocker, secret exposure, package-content gap, or external-action failure.
- Missed: section-local reviews did not catch the five additive required response keys; the planned
  whole-branch convergence gate caught and corrected them before the walkthrough, build, push, or CI.
...
- Whole-branch final review R1 (2026-09-03): additive response keys on existing callable public
  `TypedDict` values remain `NotRequired` in Python even when canonical wire responses always
  include them; runtime fixtures still require and preserve the present wire fields.
...
- Under F5, the green successor merge precedes the stable workflow dispatch; the publish workflow
  independently rechecks main ancestry, version/channel agreement, duplicate state, the full
  Python matrix, package build, and fresh installation before upload.
...
-> F5 authorization -> squash merge PR #3 -> stable Trusted Publishing -> shipped handoff
```
...
SOURCES:
`sdk-js-v0.8.0` billing contracts, credit webhook envelopes, structural-parity tests, and README
guidance for `promotion` payment/provider variants; the canonical changelog does not name this
surface.
...
- [x] B1 Prepare the non-publishing Python 0.8 candidate — Goal 3 source candidate ready for final review/build — accepted 2026-09-03 (this commit) — rounds: 0 — review: independent — profile: `goal-implementer-terra` — routing: requested=Terra/xhigh; role=gdi-implementer-terra-xhigh-v2; model/effort=runtime metadata unavailable; fallback=mapper direct Terra/medium after missing `gdi-explorer-v1` attestation — cost: ~45k tokens / 5 agents — env-retries: 0

## Completion

- [x] Every section is accepted and committed with its ledger record.
- [x] Branch is re-baselined on `origin/main` before final review; fetched
      `ca48707605ebe510273b2aa4b31faa1b73321c8b`, which was already an ancestor, so no rewrite was needed.
- [x] Whole-branch final review covers seams, contract, conformance, reader sweep, claim decay, and
      provenance; all corrections are committed before the client walkthrough or package build.
- [x] Goals 1 and 2 pass against their separately recorded canonical SHAs.
- [x] Assembled public-client walkthrough and one wheel/sdist plus fresh-install audit pass on the
      reviewed 0.8 candidate.
- [x] Goal 3 exact-head Python 3.10–3.13 CI passed on successor PR
      https://github.com/tyxter-dev/tyxter-python/pull/3 before its squash merge.
- [x] PR #2 is linked and closed only after the successor safely contains its exact artifacts.
- [x] User-authorized PR #3 squash merge `cd77d2f1bda27fc65ed92eb882d5d4b71f038c63`
      and stable publish run https://github.com/tyxter-dev/tyxter-python/actions/runs/33799345433
      completed successfully; no tag or manual package upload occurred.
- [x] Every budgeted gate records actual runs; the two F5 post-verification scope additions are
      named in Graph Findings with their initial zero count and one authorized execution.
- [x] Every deferral has a tracking URL or machine-checkable re-entry gate.
- [x] Topology graph marks match the ledger, validator is green, and the graph is re-rendered and
      compared with Graph Findings.
- [x] Routing table is complete and tokens/cost per section are reported.

## Deferrals

| ID | Class | Remaining work and risk | Owner / issue | Re-entry gate | Blocks | Status |
|---|---|---|---|---|---|---|
| D1 | external-authority | Merge the reviewed successor and publish `tyxter` 0.8.0 through Trusted Publishing | https://github.com/tyxter-dev/tyxter-python/actions/runs/33799345433 | Workflow concludes success and the PyPI 0.8.0 JSON endpoint exposes both wheel and sdist | shipped | resolved |
...
| D3 | claim-decay | PyPI 0.8.0 metadata captured the pre-publication candidate wording and cannot be replaced in place; repository docs are corrected after release | https://pypi.org/project/tyxter/0.8.0/ | Before the next canonical release upload, package metadata renders published-state wording and the built long description is reviewed | none | open |
